...
func (m *Motifini) sendVideoHandler(w http.ResponseWriter, r *http.Request) {
	...
	cam := m.SSpy.Cameras.ByName(name)
	if cam == nil {
	...
	for _, t := range strings.Split(to, ",") {
		if t == "" || !contains(m.Conf.Imessage.AllowedTo, t) {
...
func (m *Motifini) processVideoRequest(id string, cam *securityspy.Camera, to string, v map[string]string) {
	path := m.Conf.Global.TempDir + "imessage_relay_" + id + "_" + cam.Name + ".mov"
	...
	if err := cam.SaveVideo(ops, time, size, path); err != nil {
		m.Error.Printf("[%v] SaveVideo: %v", id, err)
	}
...
func (m *Motifini) sendPictureHandler(w http.ResponseWriter, r *http.Request) {
	...
	id, code, reply := ReqID(4), 200, "OK"
	path := m.Conf.Global.TempDir + "imessage_relay_" + id + "_" + name + ".jpg"
	// Check input data.
	for _, t := range to {
		if t == "" || !contains(m.Conf.Imessage.AllowedTo, t) {
			m.Debug.Printf("[%v] Invalid 'to' provided: %v", id, t)
	...
	if name == "" || code == 500 {
	...
	} else if cam := m.SSpy.Cameras.ByName(name); cam == nil {
		...
		m.Error.Printf("[%v] cam.SaveJPEG: %v", id, err)
...
func (m *Motifini) sendMessageHandler(w http.ResponseWriter, r *http.Request) {
	...
	callback := func(msg *imessage.Response) {
		if msg.Errs != nil {
			...
			m.Error.Printf("[%v] msgs.Msgs.Send '%v': %v", msg.ID, msg.To, msg.Errs)
			return
		}
		m.exports.texts.Add(1)
		m.WReq.Printf("[%v] iMessage (%d chars) sent to: %v", msg.ID, len(msg.Text), msg.To)
	}
	if code == 500 || msg == "" {
